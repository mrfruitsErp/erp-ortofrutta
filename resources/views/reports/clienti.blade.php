...
<td style="font-family:'DM Mono',monospace;font-weight:700;color:{{ $c->margin >= 0 ? 'var(--green)' : 'red' }}">
    € {{ number_format($c->margin,2,',','.') }}
</td>

<td>
    <span style="background:var(--green-xl);color:{{ $c->margin >= 0 ? 'var(--green)' : 'red' }};padding:3px 8px;border-radius:20px;font-size:12px;font-weight:600">
        {{ number_format($c->percent,1,',','.') }}%
    </span>
</td>
