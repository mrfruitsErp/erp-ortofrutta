...
<div class="kpi-grid">
    ...
    <div class="kpi-card green">
        ...
        <div class="kpi-sub">{{ $docs_count }} documenti emessi</div>
    </div>

    <div class="kpi-card orange">
        <div class="kpi-icon">📦</div>
        <div class="kpi-label">Costo Merce</div>
        <div class="kpi-value">€ {{ number_format($cost,2,',','.') }}</div>
    ...
    <div class="kpi-card blue">
        ...
        <div class="kpi-sub">{{ number_format($margin_percent,1,',','.') }}% sul fatturato</div>
    </div>

    <div class="kpi-card green">
        <div class="kpi-icon">💳</div>
        <div class="kpi-label">Crediti Clienti</div>
        <div class="kpi-value">€ {{ number_format($crediti,2,',','.') }}</div>
        <div class="kpi-sub">
            <a href="{{ url('/documents') }}">{{ $crediti_count }} documenti aperti →</a>
        </div>
    </div>
    ...
    <div class="kpi-card red">
        ...
        <div class="kpi-value">{{ $low_stock->count() }}</div>
        <div class="kpi-sub">
            <a href="{{ url('/magazzino') }}">Vedi magazzino →</a>
        </div>
    </div>
    ...
    <div class="section-card">
        ...
        <div class="section-body">
            <div class="quick-grid">
                <a href="{{ url('/documents/create') }}" class="quick-btn">
                    <span class="q-icon">📄</span> Nuovo Documento
                </a>
                <a href="{{ url('/clients/create') }}" class="quick-btn">
                    <span class="q-icon">👤</span> Nuovo Cliente
                </a>
                <a href="{{ url('/carico-magazzino') }}" class="quick-btn">
                    <span class="q-icon">🚛</span> Carica Merce
                </a>
                <a href="{{ url('/products/create') }}" class="quick-btn">
                    <span class="q-icon">🧺</span> Nuovo Prodotto
                </a>
                <a href="{{ url('/magazzino') }}" class="quick-btn">
                    <span class="q-icon">📦</span> Magazzino
                </a>
                <a href="{{ url('/movimenti-magazzino') }}" class="quick-btn">
...
<div class="dash-grid-2">
    ...
    <div class="section-card">
        <div class="section-head">
            ...
            <a href="{{ url('/report/clienti') }}">Vedi tutti</a>
        </div>
            ...
            <table class="top-table">
                ...
                @forelse($top_clients as $i => $client)
                    <tr>
                        ...
                        <td style="font-weight:600">{{ $client->client }}</td>
                        <td style="text-align:right">
                            <span class="margin-pill">€ {{ number_format($client->margin,2,',','.') }}</span>
                        </td>
                    </tr>
                @empty
    ...
    <div class="section-card">
        <div class="section-head">
            ...
            <a href="{{ url('/report/prodotti') }}">Vedi tutti</a>
        </div>
        <div class="section-body" style="padding:0">
            <table class="top-table">
                <thead>
                ...
                @forelse($top_products as $i => $product)
                    <tr>
                        ...
                        <td style="text-align:right">
                            <span class="margin-pill">€ {{ number_format($product->margin,2,',','.') }}</span>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" style="text-align:center;padding:20px;color:var(--muted)">Nessun dato</td></tr>
