...
@section('page-title', 'Modifica ' . $document->number)

@section('content')

<div class="page-header">
    <div>
        <div class="page-title">✏️ Modifica {{ $document->number }}</div>
        <div class="page-sub">Modifica cliente, data e righe del documento</div>
    </div>
    <a href="{{ url('/documents/' . $document->id) }}" class="btn btn-secondary">← Annulla</a>
</div>
...
<input type="date"
name="date"
value="{{ \Carbon\Carbon::parse($document->date)->format('Y-m-d') }}"
required>
...
<tr id="existing-row-{{ $row->id }}">

<input type="hidden" name="existing_row_id[]" value="{{ $row->id }}">
...
data-row="{{ $row->id }}"
onchange="updateExistingRow({{ $row->id }})">
...
<td class="unit">
{{ strtoupper($row->product->unit ?? '') }}
</td>

<td class="tara">
{{ number_format($row->product->tara ?? 0,3,',','.') }}
</td>
...
oninput="updateExistingRow({{ $row->id }})">
...
{{ number_format($row->kg_estimated,3,',','.') }}
