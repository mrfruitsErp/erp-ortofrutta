...
<div>{{ $document->client->city ?? '' }}</div>
</div>

<div class="info-box">
<div class="info-label">P.IVA</div>
<div class="info-value">{{ $document->client->vat ?? '—' }}</div>
</div>
...
<td class="right">
{{ number_format($row->product->tara ?? 0,2,',','.') }}
</td>

<td class="right">
{{ number_format($row->kg_real ?? $row->kg_estimated,2,',','.') }}
</td>
...
<span>€ {{ number_format($rows->sum('total'),2,',','.') }}</span>
</div>
